...
"""
class TracksView(QWidget, metaclass=Singleton):
    ...
    def create_slot_manager(self):
        self.slots_manager = VideoTrackSlotManager(self.track_offset, self.track_height, self.max_frames)
    ...
    """
    def addTrack(self, seconds, slot):
        """Add a new track to the scene."""
        number_of_frames = self.track_fps * seconds # sets the number of frames that the track has
        
        #video_track = VideoTrack(seconds)

        # Add the track item to the scene and to the list
        #self.scene.addItem(video_track.track)
        #self.video_tracks.append(video_track)

        self.slots_manager.addTrack(seconds, slot)
...
class VideoTrackSlotManager:
    ...
    """
    def __init__(self, y_offset, y_size, scene_width):
        ...
        # TracksView is a singleton
        # Initializes the "pointer" to the only TracksView object
    ...
    def addSlot(self):
        ...
        slot = VideoTrackSlot(self.current_slots, x, y, self.width, self.y_size)
...
class VideoTrackSlot(QGraphicsRectItem):
    ...
    def addTrack(self, seconds):
        ...
        seconds -> length of the track in seconds
        """
        vt = VideoTrack(seconds, self.x, self.y)
...
"""
Class VideoTrack
Represents a Video Track, which contains start time, duration in seconds, and the rectangle that represents it visually
"""
class VideoTrack:
    def __init__(self, seconds, x0, y0):
        self.duration = seconds #duration of the video track
        self.start_time = 0 #dummy value
        # Create a QGraphicsPathItem from the QPainterPath
        self.track = TrackRectangle(self, seconds, x0, y0)
...
"""
class TrackRectangle(QGraphicsPathItem):
    ...
    def __init__(self, parent_track, duration, x0, y0):
        ...
        self.snapped = None
        # VideoTrack object that holds self
    ...
    def mouseMoveEvent(self, event: QGraphicsSceneMouseEvent) -> None:
        ...
        tracks_view = TracksView() # calls the singleton
        ...
        new_slot = tracks_view.slots_manager.getSlot(new_pos.y())
        ...
        if self.current_frame < 0:
        ...
        elif self.current_frame + self.nframes >= tracks_view.max_frames:
            ...
            self.new_x = tracks_view.max_frames - self.nframes - 1
            self.current_frame = tracks_view.max_frames - self.nframes - 1
        if self.snapped:
        ...
        self.parent_track.start_time = seconds
